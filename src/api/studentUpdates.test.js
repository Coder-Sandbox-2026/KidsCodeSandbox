...
test('student transform units and coin lifecycle use real API and physics', async t => {
  ...
  cube.destroy();
  const ball = scope.createSphere();
  ball.move([10, 0, 0], 4);
  const ballRaw = api._objects.at(-1);
  assert.equal(ballRaw.physicsEnabled, true);
  assert.ok(Math.abs(ballRaw._physicsInfo.body.linearDamping() - 0.05) < 1e-6);
  assert.ok(Math.abs(ballRaw._physicsInfo.body.angularDamping() - 0.5) < 1e-6);
  assert.ok(Math.abs(ballRaw._physicsInfo.collider.friction() - 0.9) < 1e-6);
  assert.ok(ballRaw._physicsInfo.body.linvel().x > 0);
  assert.ok(Math.abs(ballRaw._physicsInfo.body.linvel().y) < 1e-12);
  ball.destroy();
  const lasting = scope.createCube();
  const expiring = scope.createSphere({ physics: true, duration: 0.02 });
  const expiringRaw = api._objects.at(-1);
  const expiringPhysicsId = expiringRaw._physicsInfo.id;
  await new Promise(resolve => setTimeout(resolve, 60));
  assert.equal(expiringRaw._destroyed, true);
  assert.equal(physics.bodies.has(expiringPhysicsId), false);
  assert.equal(api._expirations.has(expiringRaw), false);
  assert.equal(lasting._destroyed, false);
  lasting.destroy();
